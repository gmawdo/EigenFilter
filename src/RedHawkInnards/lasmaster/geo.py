...
def attibutes_prelim(x, y, z, t, config):
    ...
    time_digits = np.digitize(time, bins=times,
                              right=False)  # bins 0,1,2,...,N+1. 0 represents less than min, N+1 more than max.
    ...
    try:
        ...
        for i in np.unique(time_digits):
            ...
            k_opt, evals, evects, entropies = optimise_k(coords[time_range, :], distances, coords[indices, :], k_range,
                                                         condition)  # (num_pts, d), (num_pts,d,d)
...
def hag(coord_dictionary, config):
    ...
    unq, ind, inv, cnt = np.unique((np.floor(Coords[:, :2] / voxel_size)).astype(int), return_index=True,
